...
Sur Youdy, une application sociale qu'on a construite, on a intégré un assistant IA. La [première version](/blog/quest-ce-quun-mvp) prenait 2 à 3 secondes pour répondre — techniquement correct, mais l'expérience était frustrante. On a appris que les utilisateurs préfèrent des réponses imparfaites mais rapides plutôt que parfaites mais lentes. Du coup, on a implémenté du streaming : la réponse s'affiche progressivement, mot par mot. Techniquement, le temps total est le même. Perceptuellement, c'est instantané.
...
La performance est un moyen, pas une fin. Le but est une [expérience utilisateur](/blog/design-ux-application-mobile) fluide, pas des métriques techniques parfaites. Une animation bien faite à 60fps améliore la perception de rapidité, même si elle ajoute techniquement quelques millisecondes.
...
Mettez en place du monitoring de performance dès le lancement. [Firebase](/blog/backend-as-a-service-comparatif) Performance, New Relic, Datadog — peu importe l'outil, l'important est d'avoir de la visibilité sur ce qui se passe réellement.
