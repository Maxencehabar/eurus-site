...
C'est le standard actuel. Les API REST utilisent les méthodes HTTP classiques :
...
Alternative plus récente, GraphQL vous permet de demander exactement les données dont vous avez besoin, ni plus ni moins. Particulièrement utile quand vous avez des besoins précis et ne voulez pas charger des données inutiles.

**Avantage :** Une seule requête peut récupérer des données de plusieurs sources.
